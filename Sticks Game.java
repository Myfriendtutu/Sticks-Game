...
public class Main
{
	...
	boolean turn; //true means player's turn, false means computer turn
	if(reader.nextInt() == 1)
	turn = true;
	else
	turn = false;
	...
	//so the game must continue to run as Long as the number of sticks > 10
	while(sticks > 0){
	    if(turn)
    ...
    public static int computerMove(int sticks){
        ...
        if(sticks % 4 != 0) //if number of sticks is not a multiple of 4
            take = sticks % 4;
        else //number of sticks is multiple of 4
